...
### Fixed
- x86_64 / aarch64 改用 shadowsocks-rust 的 musl 静态发布包，避免 GNU 包因缺少 `GLIBC_2.38` / `GLIBC_2.39` 无法启动。
- 下载后先运行 `ssserver --version` 验证兼容性；运行校验或写入失败时，不再报告安装成功或登记新版本。
- 安装 / 重装时重新下载无法运行的已有 ssserver；一键检查更新支持同版本二进制修复。
- install.sh 心跳强制使用 CRLF，并在每个包管理器子步骤结束后立即恢复终端状态，避免安装期间的等待提示逐行右移。
- install.sh 在依赖安装前保存控制终端状态，并在包管理器返回后恢复，避免 `ONLCR` 被关闭导致后续文本逐行向右错位。
- install.sh 依赖安装阶段显示子步骤和每 10 秒等待提示，并将最长静默等待从 180 秒降至 120 秒；超时后 5 秒强制结束残留进程。
...
### Changed
- 将依赖自动安装移动到 install.sh bootstrap 阶段。
- 主脚本菜单内不再执行 apt/dnf/yum。
- 避免 apt/dpkg 输出与菜单提示混在一起。
- install.sh 会安装主脚本必需依赖并做二次检查。
- 缺依赖直接运行主脚本时只显示当前系统对应修复命令并停止安装，不再询问 y/N 自动安装。
- 主安装流程保持干净稳定。

## [v1.0.15] — 修复自动校准时间输出错乱

### Changed
- 修复自动校准时间输出错乱。
- 时间状态默认改为简洁块状输出。
- raw timedatectl 不再默认刷屏。
- chrony 安装命令超时但 NTP 服务已可用时不再误导性警告。
- chronyc 诊断输出与主状态分离。

## [v1.0.14] — 优化依赖自动安装终端输出
...
## [v1.0.13] — 优化 chrony sources 诊断逻辑

### Changed
- 优化 chrony sources 诊断逻辑。
- 存在 `^*` / `^+` 可用时间源时不再提示修改 `chrony.conf`。
- 仅在无可用 NTP 源时显示更换 NTP 源建议。
...
### Fixed
- 修复系统无 NTP 服务时自动校准时间无效的问题。
- 优化 `timedatectl` 显示 `NTP service: n/a` 时的提示，不再假装同步成功。

### Changed
- 网络与时间菜单中可手动确认安装 chrony，默认回车不安装。
- 一键安装 SS2022 主流程仍不安装 chrony。

## [v1.0.8] — 修复依赖自动安装误报失败

### Fixed
- 修复自动安装依赖后因包管理器返回码 / timeout 误报失败的问题。
- 自动安装后以二次依赖检查结果为准；即使包管理器返回非 0，只要必需命令已可用就继续安装。
- 优化依赖失败提示排版，减少终端错位输出。
...
### Docs
- 版本号 `v1.0.4` → `v1.0.6`；README / 状态栏 / 主菜单标题同步。
